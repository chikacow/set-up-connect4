...
pub struct ArrayBoard {
    pub cells: [Cell; WIDTH * HEIGHT], // cells are stored left-to-right, bottom-to-top
...
impl ArrayBoard {
    ...
    pub fn display(&self) -> Result<()> {
        ...
        for (idx, cell) in self.cells.iter().enumerate() {
            ...
            let (ch, fg_color) = match cell {
                Cell::PlayerOne => ("1", Color::Red),  
                Cell::PlayerTwo => ("2", Color::Blue), 
                Cell::Empty => ("0", Color::Black),
            };
            ...
            stdout
                ...
                .queue(PrintStyledContent(
                    style(ch)
                        .with(fg_color) // Set the foreground color
                        .on(Color::Black) // Set the background color
                        .attribute(Attribute::Bold), // Apply the bold attribute
                ))?;
